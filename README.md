...
Если Starvell возвращает `429`, `Too Many Requests` или `Retry-After`, защита
сначала замедляет только конкретный proxy-профиль. Общий effective limit
снижается мягко и только при явных account rate-limit заголовках или серии
повторных `429`. Если около 3 минут нет новых `429`, лимит растет на `20/мин`
обратно до `300/мин`.
